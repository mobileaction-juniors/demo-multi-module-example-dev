...
public class UserRequestQueueHandler
{
    ...
    @RabbitListener(queues = "${messaging.queue.user.request}", containerFactory = "userRequestQueueListener")
    public void handleMessage(UserRequestDto request)
    {
        ...
        {
            log.error("Could not handle user request for id: {}", request.userId(), e);
