...
public class UserResultQueueHandler
{
    ...
    @RabbitListener(queues = "${messaging.queue.userResult}", containerFactory = "userResultQueueListener")
    public void handleMessage(UserDto result)
    {
        ...
        {
            log.error("Could not handle user result for userId: {}", result.getId(), e);
